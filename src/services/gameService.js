...
export function createGame(modelId, name, players) {
  ...
  let gameName = name || 'Nouvelle partie';
  const existingNames = games.filter(
    g => g.name === gameName || g.name.startsWith(gameName + ' (')
  );
  if (existingNames.length > 0) {
    gameName = `${gameName} (${existingNames.length + 1})`;
  }
...
/**
 * Resume a game (for continuing in-progress games)
 * @param {string} id
 * @returns {Game|null}
 */
